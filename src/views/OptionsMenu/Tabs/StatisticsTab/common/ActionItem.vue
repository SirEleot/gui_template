<template>
  <div :style="{ background: `center / cover no-repeat url(/img/optionsMenu/statisticsTab/backs/${item.bg}.png), rgba(255, 255, 255, 0.05)` }" class="item">
    <div class="item__info">
      ...
      <div v-if="item.key === 'house'" class="desc">{{ property[item.key] ? 'Оплачен' : loc('mmain:stats:act:miss') }}</div>
      <div v-else-if="item.key === 'business'" class="desc">{{ property[item.key] ? `№ ${property[item.key].number}`: 'Отсутствует' }}</div>
